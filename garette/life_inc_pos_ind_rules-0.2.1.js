...
// Consts
const lirNameVerStr = "Life-Including-Positioned-Individual-Rules v0.2.1 for Web";
const lirUrlStr = "https://github.com/serhon/lipir";

const lirControlsHelpStr = "<i>Controls</i><br><br>\
0: pause to edit<br>\
1&ndash;9: speed, &emsp;&emsp; M: max speed<br>\
...
var lirCellSize = ((typeof LIPIR !== "undefined") && (typeof LIPIR.cellSize === "number")) ? Math.max(2, LIPIR.cellSize) : 11;
var lirFramerate = ((typeof LIPIR !== "undefined") && (typeof LIPIR.framerate === "number")) ? LIPIR.framerate : 32;
...
var lirShowAgecolors = ((typeof LIPIR !== "undefined") && (typeof LIPIR.showAgecolors === "boolean")) ? LIPIR.showAgecolors : true;
var lirShowRulesPositions = ((typeof LIPIR !== "undefined") && (typeof LIPIR.showRulesPositions === "boolean")) ? LIPIR.showRulesPositions : true;

var lirEmptyRulesAreBirth = ((typeof LIPIR !== "undefined") && (typeof LIPIR.emptyRulesAreBirth === "boolean")) ? LIPIR.emptyRulesAreBirth : true;
var lirAliveRulesAreDeath = ((typeof LIPIR !== "undefined") && (typeof LIPIR.aliveRulesAreDeath === "boolean")) ? LIPIR.aliveRulesAreDeath : false;
...
lirHTML += "<input type=\"checkbox\" id=\"lirCheckboxGrid\"" + (lirShowGrid ? " checked" : "") + " onChange=\"lirGridChange()\" /> <label>Grid</label><br>";

lirHTML += "<input type=\"checkbox\" id=\"lirCheckboxAgecolors\" title=\"Newer (violet) to older (red), log-rainbow palette\"" + (lirShowAgecolors ? " checked" : "") + " onChange=\"lirAgecolorsChange()\" /> <label>Agecolors</label><br>";

lirHTML += "<input type=\"checkbox\" id=\"lirCheckboxRulesPositions\"" + (lirShowRulesPositions ? " checked" : "") + " onChange=\"lirShowRulesPositionsChange()\" /> <label>Rules' positions</label></p>";

lirHTML += "<hr><p>Rules for Empty:<br>\
<label title=\"Alive cell at rule's position means Empty → Alive\"><input type=\"radio\" name=\"lirRadioEmptyRulesType\" id=\"lirRadioEmptyRulesAreBirth\" value=\"birth\"" + (lirEmptyRulesAreBirth ? " checked" : "") + " onChange=\"lirEmptyRulesTypeChange()\">Birth</label>\
&emsp;&emsp;&emsp;&emsp;\
<label title=\"Alive cell at rule's position means Empty → Empty\"><input type=\"radio\" name=\"lirRadioEmptyRulesType\" value=\"quiescence\"" + (lirEmptyRulesAreBirth ? "" : " checked") + " onChange=\"lirEmptyRulesTypeChange()\">Quiescence</label>\
</p>";

lirHTML += "<hr><p>Rules for Alive:<br>\
<label title=\"Alive cell at rule's position means Alive → Empty\"><input type=\"radio\" name=\"lirRadioAliveRulesType\" id=\"lirRadioAliveRulesAreDeath\" value=\"death\"" + (lirAliveRulesAreDeath ? " checked" : "") + " onChange=\"lirAliveRulesTypeChange()\">Death</label>\
&emsp;&emsp;&emsp;&emsp;\
<label title=\"Alive cell at rule's position means Alive → Alive\"><input type=\"radio\" name=\"lirRadioAliveRulesType\" value=\"survival\"" + (lirAliveRulesAreDeath ? "" : " checked") + " onChange=\"lirAliveRulesTypeChange()\">Survival</label>\
</p>";
...
// ShowRulesPositions changed
function lirShowRulesPositionsChange() {
	if (lirPageLoaded) {
		lirShowRulesPositions = lirCheckboxRulesPositions.checked;
		lirDrawField();
	}
}
...
function lirDrawRulesPositionsForHoverCell() {
	if (lirPageLoaded) {
		if ((lirHoverCellX >= 0) && (lirHoverCellX < lirFieldSize) && (lirHoverCellY >=0) && (lirHoverCellY < lirFieldSize)) {
			for (var i = 0; i < 9; i++) {
				var rule_pos = lirEmptyRulesPositions[lirHoverCellX][lirHoverCellX][i];
				var rX = rule_pos[1];
				var rY = rule_pos[0];
				lirDrawThickRect(rX * lirCellSize, rY * lirCellSize, lirCellSize, lirCellSize, 0x101);

				var rule_pos = lirAliveRulesPositions[lirHoverCellX][lirHoverCellX][i];
				var rX = rule_pos[1];
				var rY = rule_pos[0];
				lirDrawThickRect(rX * lirCellSize, rY * lirCellSize, lirCellSize, lirCellSize, 0x101);				
			}
...
function lirCountAliveNeighbours(y, x) {
	return lirHeaviside(lirFieldCurrent[y][(x + 1) & lirFieldSizeMask])
	       + lirHeaviside(lirFieldCurrent[(y + 1) & lirFieldSizeMask][(x + 1) & lirFieldSizeMask])
	       + lirHeaviside(lirFieldCurrent[(y + 1) & lirFieldSizeMask][x]) +
	       + lirHeaviside(lirFieldCurrent[(y + 1) & lirFieldSizeMask][(x - 1) & lirFieldSizeMask])
	       + lirHeaviside(lirFieldCurrent[y][(x - 1) & lirFieldSizeMask])
	       + lirHeaviside(lirFieldCurrent[(y - 1) & lirFieldSizeMask][(x - 1) & lirFieldSizeMask])
	       + lirHeaviside(lirFieldCurrent[(y - 1) & lirFieldSizeMask][x])
	       + lirHeaviside(lirFieldCurrent[(y - 1) & lirFieldSizeMask][(x + 1) & lirFieldSizeMask]);
}
...
function lirDrawField() {
	...
	if (lirShowRulesPositions) {
		lirDrawRulesPositionsForHoverCell();
	}
...
// Determine cell, over which pointer is now
lirCanvas.addEventListener("mousemove", function(event) {
	if (lirPageLoaded) {
		lirHoverCellX = Math.floor(event.offsetX / lirCellSize);
		lirHoverCellY = Math.floor(event.offsetY / lirCellSize);
...
lirCanvas.addEventListener("mouseleave", function(event) {
	if (lirPageLoaded) {
		lirHoverCellX = -1;
		lirHoverCellY = -1;
